...
int neighborhood[][2] = {{-6, -6}, {-6, -3}, {-6, 0}, {-6, 3},
                         {-3, -6}, {-3, -3}, {-3, 0}, {-3, 3},
                         {-3, 6},  {0, -6},  {0, -3}};

int upper_neighborhood[][2] = {{-3, -3}, {-3, 0}, {-3, 3}, {0, -3}, {0, 0},
                               {0, 3},   {3, -3}, {3, 0},  {3, 3}};
 ...
 * pyramid.
 *
 * This function returns the neighborhood of a specific pixel at level L in the
 * pyramid, including the pixel's neighbors at the current level and possibly at
 * an upper pyramid level.
 *
 ...
 *
 * @return Pointer to an array of RGB pixel values representing the neighborhood
 * of pixel (x, y). Includes values from both the current and upper levels (if
 * applicable).
 */
int *build_neighborhood(Image *G, int L, int x, int y) {
